...
class WarehouseController extends AbstractController
{
    ...
    /**
     * @Route("/change", name="warehouse_change", methods={"GET"})
     */
    public function change(WarehouseRepository $warehouseRepository,EntityManagerInterface $entityManager): Response
    {
        if(isset($_GET['submit'])){
            $warehouse = $warehouseRepository->findOneByName($_GET['warehouse']);
            $user = $this->getUser();
            var_dump($warehouse->getName());
            $user->setActiveWarehause($warehouse);
            $entityManager->persist($user);
            $entityManager->flush();
            return $this->redirectToRoute('dashboard', [], Response::HTTP_SEE_OTHER);
        }
        return $this->render('warehouse/change.html.twig', [
            'warehouses' => $warehouseRepository->findAll(),
        ]);
    }
